...
namespace barcodescanner.Classes
{
    ...
    /// Fixed: commands created while an ambient transaction is active will be bound to that transaction.
    /// </summary>
    public class SqlDatabase : IDatabase, IDisposable
    {
        private readonly SqlConnection conn;
        private SqlTransaction ambientTransaction; // <<-- NEW: holds currently active transaction for command binding
        ...
        /// <summary>
        /// Create a SqlCommand. If tran is supplied it will be used; otherwise if an ambientTransaction exists
        /// it will be attached to the created command so commands executed within ExecuteTransaction(...) are bound.
        /// </summary>
        private SqlCommand CreateCommand(string query, object parameters = null, SqlTransaction tran = null)
        {
            EnsureConnectionOpen();
            // prefer explicit tran; otherwise fall back to ambientTransaction (if any)
            var cmd = tran != null ? new SqlCommand(query, conn, tran) : new SqlCommand(query, conn);
            if (tran == null && ambientTransaction != null)
        ...
        public int SaveReturnID(string table, object data)
        {
            var props = data.GetType().GetProperties();
    ...
    ";
            ...
            using (var cmd = CreateCommand(query, data))
            {
        ...
        /// This implementation sets an ambientTransaction so CreateCommand() attaches it automatically.
        /// </summary>
        public bool ExecuteTransaction(Func<SqlTransaction, bool> work)
        {
            ...
            using (var tran = conn.BeginTransaction())
            {
                bool previousAmbient = ambientTransaction != null;
                var savedAmbient = ambientTransaction;
                try
                {
                    // Set ambient so helper methods pick it up
                    ambientTransaction = tran;
                ...
                {
                    // restore prior ambient (if any) - usually null
                    ambientTransaction = savedAmbient;
                }
            }
        }
    ...
    SELECT COUNT(*) FROM {Q(table)} {whereClause};";
        ...
        public bool BulkInsert(string table, DataTable data)
        {
            ...
            try
            {
                ...
                {
                    // If there is an ambient transaction, attach it to bulk copy
                    if (ambientTransaction != null)
                        bulk.SqlRowsCopied += (s, e) => { }; // no-op, but SqlBulkCopy doesn't allow setting Transaction property in some providers
